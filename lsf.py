...
class LimitStateFunction:
    ...
    def ode_solve(self, theta):
        ...
        for sim in range(theta.size(0)):  # Can be parallelized
            ...
            oscillator = HystereticOscillator(u_j[sim, :], u_hat_j[sim, :]).to(device)  # Move model to device
    ...
    def ode_solve2(self, theta): # theta here should be the random variables
        device = theta.device  # Get the device of theta
        ...
        for sim in range(theta.size(0)):  # Can be parallelized
            ...
            oscillator = hyperbolic(u[sim, :], v[sim, :]).to(device)  # Move model to device
